...
import com.donneryst.popularmovies.model.Review;
...
/**
 * The task is used for downloading all the reviews of a particular movie from theMovieDB API.
 *
 * Author: jhpx
 * Create: 2016/3/22
 */
public class FetchReviewsTask extends CommonHttpTask<Integer, Void, List<Review>> {


    public FetchReviewsTask(Context context, AsyncTaskListener<List<Review>> listener) {
    ...
    protected List<Review> doInBackground(Integer... params) {
        ...
        try {
            ...
            Logger.v(response);
            Type listType = new TypeToken<Result<Review>>() {
            }.getType();
